...
lower_bound = {"CaO":391.95, "MgO": 279.43, "Al2O3": 308.65, "SiO2": 286.5, "Na2O":588.5, "TiO2": 334.5, "CaI": 421}
upper_bound = {"CaO":395.66, "MgO": 279.66, "Al2O3": 309.9, "SiO2": 290, "Na2O":589.5, "TiO2":335.36, "CaI": 423}
...
if aux == 2:
    ...
    for i, (sample_file, sample_data) in enumerate(samples.items()):
        ...
        if compound== "CaO":
            x2, y2, main_peak2, _, area_under_peak2 = process_spectrum(sample_data['wavelengths'], sample_data['intensities'], lower_bound["CaI"], upper_bound["CaI"])       
        ...
        if area_under_peak and ref_area_under_peak and compound != "CaO":
        ...
        if compound =="CaO":
            area_ratio = area_under_peak2 /area_under_peak
            print(f"Total Area under Peak: {area_under_peak}")
            print(f"Area Ratio: {area_ratio}")
            area_ratios.append(area_ratio)
            Sum_fwhm_vect.append(area_under_peak)
